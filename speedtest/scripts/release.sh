#!/bin/bash
# nodecoop-speedtester 一键发布:bump VERSION -> 更新 README changelog -> commit -> tag(speedtest-vX.Y.Z) -> push -> 创建 GitHub Release
...
PLUGIN_DIR="$(dirname "$SCRIPT_DIR")"   # .../speedtester/speedtest
REPO_ROOT="$(dirname "$PLUGIN_DIR")"    # .../speedtester
cd "$REPO_ROOT"
...
RELEASE_BODY="## nodecoop-speedtester ${TAG}
